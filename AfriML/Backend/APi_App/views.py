...
training_dir = os.path.join("static", "data", "training")
validation_dir = os.path.join("static", "data", "validation")
        ...
        try:
            ...
            for i in range(len(classes)):
                ...
                if main_flag == "image_classification" and dir_name != "":
                    os.makedirs(os.path.join(training_dir, dir_name), exist_ok=True)
    ...
    if request.method == 'POST':
        try:
            ...
            if main_flag == 'text_classification':
                ...
                os.remove(os.path.join("static", "train_text.csv"))
                csv_file_save_path = os.path.join("static", "train_text.csv")
                ...
                for i in range(len(images_list)):
                    if main_flag == "image_classification":
                        if i % 4 == 0 and i != 0:
                            fs = FileSystemStorage()
                            file = fs.save(os.path.join(validation_dir, class_name, f'{i}.jpg'), images_list[i])
                        else:
                            fs = FileSystemStorage()
                            file = fs.save(os.path.join(training_dir, class_name, f'{i}.jpg'), images_list[i])
                        ...
                        fs = FileSystemStorage()
                        file = fs.save(os.path.join(training_dir, class_name, f'{i}.wav'), images_list[i])
                    else:
                        pass
    ...
    if request.method == 'POST':
        try:
            ...
            if main_flag == 'pose_classification':
                ...
                if flag == 'train':
                    ...
                    try:
                        if request.FILES.get("image") is not None:
                            ...
                            delete_files_in_directory(os.path.join('static', 'input_images'))

                            fs = FileSystemStorage()
                            image_path = os.path.join('static', 'input_images', 'test.jpg')
                            file = fs.save(image_path, image)

                            if os.path.exists(csv_path) and os.path.exists(image_path):
                                image = cv2.imread(image_path)
                                class_res, confidences, all_classes = test_image(csv_path, image)
                    ...
                    model_path = os.path.join('static', 'final_model.h5')
                    host = request.get_host()
                    data_path = os.path.join(host, 'static', 'data.csv')
                    if os.path.exists(model_path):
                        context = {
                            "status": "success",
                            "Model": f"{os.path.join(host, model_path)}",
                            "Data": f"{data_path}"
                        }
                    else:
                        context = {
                ...
                flag = request.POST.get("flag")
                if flag == 'train':
                    classification_data = os.path.join('static', 'data')
                    ...
                    try:
                        if request.FILES.get("image") is not None:
                            ...
                            if os.path.exists(classification_data) and os.path.exists(image_path):
                                ...
                                class_res, confidences, classes_names = predict_image_class(image_path, 'static/image_classification_classes.json')
                                context = {
                                    "status": "success",
                                    "Predicted Class": class_res,
                                    "All Classes": classes_names,
                                    "Confidences": confidences
                                }
                            else:
                                context = {
                    ...
                    model_path = os.path.join('static', 'image_classification_model.pkl')
                    host = request.get_host()
                    classes_path = os.path.join(host, 'static/image_classification_classes.json')
                    if os.path.exists(model_path):
                        context = {
                            "status": "success",
                            "Model": f"{os.path.join(host, model_path)}",
                ...
                flag = request.POST.get("flag")
                if flag == 'train':
                    classification_data = os.path.join('static', 'train_text.csv')
                    if os.path.exists(classification_data):
                    ...
                    try:
                        if request.POST.get("text") is not None:
                            ...
                            if os.path.exists(classification_data):
                                ...
                                res_status, class_res, classes = text_inference(input_text)
                                ...
                                if res_status is True:
                                    context = {
                                        "status": "success",
                                        "Predicted Class": class_res["label"],
                    ...
                    model_path = os.path.join("static", "text_classification_model")
                    zip_directory(model_path, f"{model_path}.zip")
                    host = request.get_host()
                    saved_model_path = os.path.join(host, model_path+'.zip')
                    classes_path = os.path.join(host, 'static', 'train_text_classes.json')
                    if os.path.exists(model_path):
                        context = {
                            "status": "success",

                            "Model": f"{saved_model_path}", 
                            "Classes": f"{classes_path}"
                        }
                    else:
                        context = {
                            "status": "failed",
                ...
                flag = request.POST.get("flag")
                if flag == 'train':
                    classification_data = os.path.join('static', 'data', 'training')
                    if os.path.exists(classification_data):
                    ...
                    try:
                        if request.FILES.get("image") is not None:
                            ...
                            fs = FileSystemStorage()
                            delete_files_in_directory(os.path.join('static', 'input_audios'))
                            audio_path = os.path.join('static', 'input_audios', 'test.wav')
                            ...
                            if os.path.exists(classification_data) and os.path.exists(audio_path) and os.path.exists(model_path):
                                ...
                                res_status, class_res, confidences, all_classes = inference_audio_file(classification_data, model_path, audio_path)
                    ...
                    model_path = os.path.join('static', 'weights', 'audio_model.h5')
                    host = request.get_host()
                    host_model_path = os.path.join(host, model_path)
                    data_path = os.path.join(host, "static", "dataset", "data.csv")
                    classes_path = os.path.join(host, "static", "dataset", "classes.txt")

                    if os.path.exists(model_path):
                        context = {
                            "status": "success",
                            "Model": f"{host_model_path}",
                            "Data": f"{data_path}",
                            "Classes": f"{classes_path}"

                            # "Model": f"Download at:\n{host_model_path}",
                            # "Data": f"Download at:\n{data_path}",
                            # "Classes": f"Download at:\n{classes_path}"
                        }
                    else:
                        context = {
                            "status": "failed",
